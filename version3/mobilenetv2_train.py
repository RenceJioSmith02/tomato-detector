...
# This prevents low-confidence detections from overriding healthy/late_blight.
#
# Priority order: late_blight -> healthy -> other_diseases (last resort)
#
# Rationale:
#   - other_diseases is a catch-all for diseases not in the training set.
#     It should only fire when the model is fairly confident AND neither
#     late_blight nor healthy clears their thresholds.
#   - Setting a high OD floor (0.60) keeps other_diseases as a genuine
#     prediction rather than a low-confidence fallback.
...
#   1. late_blight first — spreads fast, high crop damage risk, must not be missed.
#   2. healthy second — the model should prefer confirming health over flagging
...
print(f"Minimum threshold floors: late_blight >= {MIN_LB_THRESHOLD}, "
      f"other_diseases >= {MIN_OD_THRESHOLD}")
...
def predict_with_thresholds(scores_array, lb_thresh, od_thresh):
    ...
    for scores in scores_array:
        ...
        elif scores[HEALTHY_IDX] >= od_thresh:
...
best_macro_f1  = 0.0
best_lb_thresh = MIN_LB_THRESHOLD
best_od_thresh = MIN_OD_THRESHOLD

# Coarse 2D grid search (step=0.05)
# Search starts at the minimum floors — never below them.
print("Running coarse 2D grid search...")
for lb_t in np.arange(MIN_LB_THRESHOLD, 0.90, 0.05):
    for od_t in np.arange(MIN_OD_THRESHOLD, 0.90, 0.05):
...
# Clamp lower bound to the minimum floors.
print("Fine-tuning around best coarse point...")
for lb_t in np.arange(max(MIN_LB_THRESHOLD, best_lb_thresh - 0.06),
                       best_lb_thresh + 0.07, 0.01):
    for od_t in np.arange(max(MIN_OD_THRESHOLD, best_od_thresh - 0.06),
...
print(f"  Late Blight threshold: {best_lb_thresh:.2f}  (floor: {MIN_LB_THRESHOLD})")
print(f"  OD threshold:          {best_od_thresh:.2f}  (floor: {MIN_OD_THRESHOLD})")
...
threshold_config = {
    ...
    "min_lb_threshold_floor":  MIN_LB_THRESHOLD,
    "min_od_threshold_floor":  MIN_OD_THRESHOLD,
    "note": (
        "Priority: if late_blight score >= late_blight_threshold -> late_blight. "
        "Elif healthy score >= od_threshold -> healthy. Else -> other_diseases. "
        "other_diseases is last resort — thresholds never go below their floor values."
    )
}
...
print(f"Late Blight threshold:       {best_lb_thresh:.2f}  (floor: {MIN_LB_THRESHOLD})")
print(f"OD threshold:                {best_od_thresh:.2f}  (floor: {MIN_OD_THRESHOLD})")
print(f"Priority order:              late_blight -> healthy -> other_diseases")
print("-"*70)
